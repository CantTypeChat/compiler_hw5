...
int literal_no=0; int literal_size=0;
...
A_TYPE *sem_expression(A_NODE *node)
{
    ...
    switch(node->name) {
        case N_EXP_IDENT:
            // makeNode(N_EXP_IDENT, 0, getIdentifierDeclared, 0);
            // when the code is:
            //
            // struct s st;
            // st.a = 10;
            //
            // st -> IDENTIFIER -> primary(st.kind==ID_VAR) -> postfix
            // . -> DOT
            // a -> IDENTIFIER (string "a")
            // postfix DOT IDENTIFIER {makeNode(N_EXP_STRUCT, $1, 0, $3);
            // we don't know about a's ID, but string "a".
            // we should set a's ID in semantic analysis
            // when node->name == N_EXP_STRUCT
            id = node->clink;
            switch (id->kind) {
                case ID_VAR:
                case ID_PARM:
        ...
        case T_ENUM:
            ...
            while(id) {
                if(id->init) {
                    ...
                    if(!isIntType(lit.type)) // enum A {ONE = 1.0, TWO};
...
// get ID table of the member of the struct's name == s
...
// TODO: check if this function is called when node->type != float.
A_NODE *convertScalarToInteger(A_NODE *node)
{
    if(isFloatType(node->type)) { // if it was a float, print warning.
...
BOOLEAN isAllowableCastingConversion(A_TYPE *t1, A_TYPE *t2)
{
    if(isAnyIntegerType(t1) && (isAnyIntegerType(t2) || isFloatType(t2) || isPointerType(t2)))
        ...
        return TRUE;
    else if (isVoidType(t1)) // TODO: ??? I don't understand what is this code for.
        return TRUE;
    else
        return FALSE;
}
